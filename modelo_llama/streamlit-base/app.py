...
def query_local_model(message, session_id="", model_params=None, context="", conversation_history=None):
    ...
    try:
        ...
        result = invoke_local_model(prompt, model_params)
...
def handle_message():
    ...
    if st.session_state.user_input.strip():
        ...
        # Definir current_session_id ANTES de qualquer uso
        is_first_message = len(st.session_state.messages) == 0
        current_session_id = "" if is_first_message else st.session_state.session_id
        
        user_message = " ".join(st.session_state.user_input.strip().split())
        ...
        if not is_duplicate:
            ...
            with st.chat_message("assistant", avatar=logo_path):
                ...
                with st.spinner():
                    ...
                    result = query_local_model(user_message, current_session_id, context=combined_context, conversation_history=st.session_state.messages)
